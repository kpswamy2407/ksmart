				...
				switch (input.ldapentitytype) {
					...
					case 'distributorprofile':
						redisHelper.getValue(req.query.auth_token + "_" + req.ip).then(validToken => {
							...
							sequelize.query(authConfig.getKey("distributorprofilesql").replace("$whereCondition", queryString), {
								type: sequelize.QueryTypes.SELECT
							}).spread(queryRes => {
					...
					case 'salesmanprofile':
						redisHelper.getValue(req.query.auth_token + "_" + req.ip).then(validToken => {
							...
							queryString=queryString.trim(" ").slice(0, -3)
							sequelize.query(authConfig.getKey("salesmanprofilesql").replace("$whereCondition", queryString), {
								type: sequelize.QueryTypes.SELECT
							}).spread(queryRes => {
								if (queryRes.isactive != 1) {
									throw new HttpError(401, 'ERR-AU-0001', 'Inactive User')
...
routes.get('/test', (req, res, next) => {
	res.json({
		'status': 'Node application started'
	});
})
